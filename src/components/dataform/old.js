...
class DataForm extends Component {
    ...
    queryType(antd_form, querys, reportId, getExcel, buttonState) {
        ...
        querys.forEach(function (query) {
            if (query.fieldDataType === 'NUMBER') {
                controls.push(
                    <Col span={8}>
                        <NumberComponent form={antd_form} data={query} layout={formItemLayout}/>
                    </Col>
                )
            ...
            } else if (query.fieldDataType === 'VARCHAR') {
                controls.push(
                    <Col span={8}>
                        <StringComponent form={antd_form}
                                         data={query} layout={formItemLayout}
                                         onFetchUnionSelect={_this.fetchUnionSelect.bind(_this, query.chainedPosValue, reportId)}/>
                    </Col>
                )
            } else if (query.fieldDataType === 'YEAR') {
                controls.push(
                    <Col span={8}>
                        <YearComponent form={antd_form} data={query} layout={formItemLayout}/>
                    </Col>
                )
            }
        });
